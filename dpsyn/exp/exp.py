...
from dpsyn.lib_attrs.attr_recode import AttrRecord
from dpsyn.lib_view.marg_select import MarginalSelection
from dpsyn.lib_dataset.data_store import DataStore
...
class Exp:
    def __init__(self, args):
        ...
        self.load_data()
        self.logger.info(
            "original dataset domain: %e" % (self.original_dataset.domain.size(),)
        )
        ...
        if self.args["noise_add_method"] == "Non":
            self.gauss_sigma = 0.0
        else:
            self.gauss_sigma = self._calculate_sigma(
                self.remain_epsilon, len(self.marginals) + self.num_attributes
            )
    ...
    def privacy_budget_allocation(self):
        ...
        self.logger.info(
            "privacy budget allocation: marginal %s | synthesize %s"
            % (self.depend_epsilon, self.remain_epsilon)
        )
    ...
    def select_marginals(self, dataset):
        if self.args["is_cal_marginals"]:
            ...
            marginal_selection = MarginalSelection(dataset, select_args)
    ...
    def _calculate_rho(self, epsilon):
        ...
        sigma = composition.gauss_zcdp(
            epsilon, self.delta, self.args["marg_add_sensitivity"], 1
        )
    ...
    def _calculate_sigma(self, epsilon, num_views):
        ...
        return composition.gauss_zcdp(
            epsilon, self.delta, self.args["marg_add_sensitivity"], num_views
        )
