...
from dpsyn.lib_view.consistent import Consistenter
...
class ExpDPSyn(Exp):
    ...
    def construct_view(self, dataset, marginal):
        num_keys = reduce(
            lambda x, y: x * y, [dataset.domain.config[m] for m in marginal]
        )
    ...
    def anonymize_view(self, view, rho=0.0, epsilon=0.0):
        if self.epsilon != -1.0:
            if rho != 0.0 and epsilon == 0.0:
                ...
                view.count += np.random.laplace(
                    scale=self.args["marg_add_sensitivity"] / epsilon, size=view.num_key
                )
