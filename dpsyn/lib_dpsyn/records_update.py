...
class RecordUpdate:
    ...
    def initialize_records(self, iterate_keys, method="random", singletons=None):
        self.records = np.empty(
            [self.num_records, len(self.domain.attrs)], dtype=np.uint32
        )
        self.df = pd.DataFrame(self.records, columns=self.domain.attrs)

        for attr in self.domain.attrs:
            if method == "random":
                self.df[attr] = np.random.randint(
                    0, self.domain.config[attr], size=self.num_records
                )
    ...
    def update_records_main(self, view, alpha):
        ...
        self.cell_under_indices = np.where(
        ...
        self.cell_zero_indices = np.where(
            (self.synthesize_marginal == 0) & (self.actual_marginal != 0)
        )[0]
        self.num_add_zero = self.normal_rounding(
            alpha * self.actual_marginal[self.cell_zero_indices] * self.num_records
        )

        #################################### deal with cells to be reduced ########################################
        # determine the number of records to be removed
        self.cell_over_indices = np.where(
            self.synthesize_marginal > self.actual_marginal
        )[0]
        ...
        ratio_reduce = np.minimum(
            ...
            / self.synthesize_marginal[self.cell_over_indices],
            np.full(self.cell_over_indices.shape[0], beta),
        )
        ...
        self.logger.debug(
            "num_boost: %s | num_reduce: %s" % (num_add_total, np.sum(self.num_reduce))
        )

        # calculate some general variables
        self.encode_records = np.matmul(
            self.records[:, view.attributes_index], view.encode_num
        )
    ...
    def complete_partial_ratio(self, view, complete_ratio):
        ...
        for valid_index, cell_index in enumerate(valid_cell_under_indices):
            match_records_indices = self.encode_records_sort_index[
                valid_data_under_index_left[valid_index] : valid_data_under_index_right[
                    valid_index
                ]
            ]
            ...
            if self.records_throw_indices.shape[0] >= (
                ...
                if num_complete[valid_index] != 0:
                    ...
                    ] = self.records[match_records_indices[: num_complete[valid_index]]]
    ...
    def find_optimal_beta(self, num_add_total, cell_over_indices):
        ...
        while abs(num_add_total - current_num) >= 1.0:
            ...
            current_num = np.sum(
                np.minimum(
                    (synthesize_marginal_under - actual_marginal_under)
