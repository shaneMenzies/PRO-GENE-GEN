...
from dpsyn.lib_composition.advanced_composition import AdvancedComposition
...
class AttrDepend:
    ...
    def transform_records_distinct_value(self):
        ...
        self.dataset_domain.config = dict(
            zip(self.dataset_domain.attrs, distinct_shape)
        )
    ...
    def calculate_pair_indif(self):
        ...
        for first_index, first_attr in enumerate(self.dataset_domain.attrs[:-1]):
            first_view = View(
                self.dataset_domain.project(first_attr), self.dataset_domain
            )
            ...
            for second_attr in self.dataset_domain.attrs[first_index + 1 :]:
                ...
                second_view = View(
                    self.dataset_domain.project(second_attr), self.dataset_domain
                )
                ...
                pair_view = View(
                    self.dataset_domain.project((first_attr, second_attr)),
                    self.dataset_domain,
                )
                ...
                normalize_pair_view_count = pair_view.count_matrix / np.sum(
                    pair_view.count_matrix
                )
                error = np.sum(
                    np.absolute(
                        normalize_pair_view_count - independent_pair_distribution
                    )
                )

                num_cells = (
                    self.original_domain.config[first_attr]
                    * self.original_domain.config[second_attr]
                )
                dependency_df.loc[dependency_index] = [
                    first_attr,
                    second_attr,
                    num_cells,
                    error,
        ...
        pickle.dump(
            dependency_df, open(config.DEPENDENCY_PATH + self.dataset_name, "wb")
        )
    ...
    def solve_score_function(
        self,
        dataset_name,
        epsilon,
        rho,
        marg_add_sensitivity,
        marg_select_sensitivity,
        noise_add_method,
    ):
        self.logger.info("choosing marginals")

        self.dependency_df = pickle.load(
            open(config.DEPENDENCY_PATH + self.dataset_name, "rb")
        )
        ...
        if epsilon != 0.0:
            ...
            sigma = composition.gauss_zcdp(
                epsilon,
                1.0 / (self.df.shape[0]),
                marg_select_sensitivity,
                self.dependency_df.shape[0],
            )
            self.dependency_df.error += np.random.normal(
                scale=sigma / (2.0 * self.df.shape[0]), size=self.dependency_df.shape[0]
            )
        ...
        while gap > threshold and len(self.marginals) <= max_num:
            ...
            for j in unselected:
                ...
                if noise_add_method == "A3":
                    ...
                    gauss_constant = np.sqrt(
                        (marg_add_sensitivity**2 * len(select_candidate)) / (2.0 * rho)
                    )
                    gauss_error = np.sum(
            ...
            first_attr, second_attr = (
                self.dependency_df.loc[selected_index, "first_attr"],
                self.dependency_df.loc[selected_index, "second_attr"],
            )
            self.marginals.append((first_attr, second_attr))
            self.selected_attrs.update((first_attr, second_attr))

            self.logger.info(
                "select %s marginal: %s | gap: %s"
                % (len(self.marginals), (first_attr, second_attr), gap)
            )
        ...
        for attr in missing_attrs:
            if method == "isolate":
                ...
                match_missing_df = self.dependency_df.loc[
                    (self.dependency_df["first_attr"] == attr)
                    | (self.dependency_df["second_attr"] == attr)
                ]
                match_df = match_missing_df.loc[
                    (match_missing_df["first_attr"].isin(self.selected_attrs))
