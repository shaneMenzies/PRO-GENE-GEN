    ...
    def norm_cut(self):
        ...
        if positive_indices.size != 0:
            ...
            if threshold_indices.size == 0:
                self.count[positive_indices[positive_sort_indices[0]]] = (
                    sort_cumsum[0] - negative_total
                )
            else:
                self.count[
                    positive_indices[positive_sort_indices[threshold_indices]]
                ] = 0.0
                next_index = threshold_indices[-1] + 1

                if next_index < positive_sort_indices.size:
                    self.count[positive_indices[positive_sort_indices[next_index]]] = (
