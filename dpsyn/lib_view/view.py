...
from dpsyn.lib_view.non_negativity import NonNegativity
...
class View:
    def __init__(self, view_domain, dataset_domain):
        ...
        self.indicator[
            [dataset_domain.attr_index_mapping[attr] for attr in view_domain.attrs]
        ] = 1
    ...
    def project_from_bigger_view_general(self, bigger_view):
        ...
        self.count = np.bincount(
            encode_tuple_key, weights=bigger_view.count, minlength=self.num_key
        )
    ...
    def project_from_bigger_view(self, bigger_view, index):
        ...
        self.weights[index] = 1.0 / np.product(
            self.num_categories[
                np.setdiff1d(bigger_view.attributes_index, self.attributes_index)
            ]
        )
    ...
    def update_view(self, common_view, index):
        ...
        np.add.at(
            self.count, sort_indices, np.repeat(common_view.delta[:, index], count)
        )
