...
data <- read_csv('./outputs/coex_data_k5.csv')
...
data <- read_csv('./outputs/coex_data_k1000.csv')
real <- dplyr::filter(data, set == 'real')
data <- dplyr::filter(data, !set == 'real')
print(real$correctly_rec[1])
...
png(file = paste0('./outputs/coex-plot.png'), bg = "transparent", 
...
write_csv(data, './outputs/coex-data-summarised.csv')
