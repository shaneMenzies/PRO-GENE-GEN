...
class RONGauss:
    ...
    def __init__(
        self, z_dim, target_epsilon, target_delta, conditional, enable_privacy
    ):
    ...
    def generate(
        ...
        if self.conditional:
            return self._gmm_rongauss(
                X, y, n_samples, reconstruct, prng_seed, if_uniform_y
            )

        else:
            return self._supervised_rongauss(
                X, y, n_samples, max_y, reconstruct, centering, prng_seed
            )

    def _gmm_rongauss(self, X, y, n_samples, reconstruct, prng_seed, if_uniform_y):
        ...
        for label in np.unique(y):
            ...
            x_class = X[idx]
            (x_bar, mu_dp) = self._data_preprocessing(
                x_class, self.epsilon_mean, prng, self.enable_privacy
            )
            ...
            if if_uniform_y:
                ...
                synth_data = prng.multivariate_normal(mu_dp_tilda, cov_dp, len(x_class))
                syn_y = np.append(syn_y, label * np.ones(len(x_class)))
    ...
    @staticmethod
    def _data_preprocessing(X, epsilon_mean, prng=None, enable_privacy=True):
        if prng is None:
