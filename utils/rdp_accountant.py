...
__all__ = ["compute_sigma", "compute_epsilon"]
...
def compute_sigma(epsilon, sampling_prob, iterations, delta, verbose=False):
    func = (
        lambda x: compute_epsilon(x, sampling_prob, iterations, delta, verbose=verbose)
        - epsilon
    )
...
if __name__ == "__main__":
    SIGMA = 1.07  # noise
    SAMPLING_PROB = 256 / 50000  # Sampling prob. for SGD
    SENSITIVITY = 1  # SENSITIVITY = 6.0
    T = 1000 * 1 * 10  # Total iterations
    DELTA = 10**-6  # DP parameter; we fix delta and compute the minimum epsilon
    eps = 5
    print(eps)
